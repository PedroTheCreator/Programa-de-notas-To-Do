...
def apresentar():
  ...
  x = int(input("Insira sua escolha: "))
  os.system('clear')
  return x
...
while(escolha != 5):
  ...
  arq = open("notas.txt","r") # Abro o arquivo em modo leitura
  ...
  try:
    ...
    limpar_nota("notas.txt", True) # Cria o contador iniciando em 0
  ...
  if escolha == 1:
    ...
    limpar_nota("notas.txt", False) # Limpo a nota, sem deixar o texto padrão.
    arq = open("notas.txt", "a+") # Abro no modo adição no fim.
    ...
    for i in conteudo:
      print(i)
    apagador = int(input("Insira a nota que deseja apagar: ")) # Recebo o índice
    conteudo.pop((apagador-1)) # Apago da lista, com descréscimo, pois a lista começa em 0.
    total-=1 #Decréscimo no total
    limpar_nota("notas.txt",False) # Limpo a nota sem a entrada padrão. Consulte "Funções para organização" no slide.
    arq = open("notas.txt", "a+") # Abro o arquivo em modo adição no final.
    ...
    if confirmacao.lower() == "sim": # Comparação.
      limpar_nota("notas.txt", True) # Função de organização.
      conteudo.clear() # Limpeza na lista recebida da rotina de loop.
      total = 0 # Zerar o contador.
      print("Notas limpas com sucesso!") # Feedback ao usuário.
      limpar_tela() # Função de organização.
    else:
      print("Suas notas foram mantidas.")
      limpar_tela() # Função de organização.
  elif escolha == 5:
